...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml"  version="2.0" 
	...
	<xsl:template match="ukm:EUMetadata">
		<div class="assocDocs filesizeShow colSection p_one s_7">
			<h3><xsl:value-of select="leg:TranslateText('Dates')"/></h3>
			...
			<xsl:choose>
				<xsl:when test="exists(ukm:EnactmentDate) or exists(*:RESOURCE_LEGAL_DATE_ENTRY-INTO-FORCE)  or exists(*:RESOURCE_LEGAL_DATE_END-OF-VALIDITY) or exists(*:RESOURCE_LEGAL_DATE_SIGNATURE)">
					<div>
						<ul class="plainList">
				...
				<xsl:otherwise>
					<!-- default message -->
					<p><xsl:value-of select="leg:TranslateText('There are no dates avaialable for this legislation')"/></p>
				</xsl:otherwise>
			</xsl:choose>
		</div>
	</xsl:template>
	...
	<xsl:template match="*:RESOURCE_LEGAL_DATE_ENTRY-INTO-FORCE | *:RESOURCE_LEGAL_DATE_END-OF-VALIDITY | *:RESOURCE_LEGAL_DATE_SIGNATURE">
		<xsl:variable name="date">
			<xsl:text>Date of </xsl:text>
			<xsl:value-of select="if (self::*:RESOURCE_LEGAL_DATE_END-OF-VALIDITY) 
										then ' end of validity: ' 
									else if  (self::*:RESOURCE_LEGAL_DATE_SIGNATURE) 
										then ' signature: '
									else 'effect: '"/>
		  ...
		  <!--/#breadcrumbControl --> 
			<div id="breadCrumb">
				...
				<ul>
					...
					<li class="activetext"><xsl:value-of select="leg:TranslateText('In Force Information')"/></li>
				</ul>
		</div>
	</xsl:template>
